...
### Official OIDF suite installation

The official OpenID Foundation Conformance Suite is pinned and runnable locally via
`tools/oidf/run-local-suite.sh` at release `release-v5.2.1` (commit
`932b46f1e507871eb0b34621aaef65ff04442e6f`). The suite UI is exposed at `https://localhost:8443`.
This records installation only; issuer and verifier test plans still require a configured,
externally reachable deployment and are not represented as passed evidence here.
